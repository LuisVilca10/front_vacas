...
abstract class VacaApi {
  ...
  @POST("/vaca/crear")
  Future<VacaRespModelo> crearVaca(@Header("Authorization") String token, @Body() VacaModelo vaca);

  @GET("/vaca/buscar/{id}")
  Future<VacaModelo> findVaca(@Header("Authorization") String token, @Path("id") int id);

  @DELETE("/vaca/eliminar/{id}")
  Future<GenericModelo> deleteVaca(@Header("Authorization") String token, @Path("id") int id);

  @PUT("/vaca/editar/{id}")
  Future<VacaModelo> updateFinca(@Header("Authorization") String token, @Path("id") int id , @Body() VacaModelo vaca);
}